            ...
            < >
                <tr key={ uniqid() } >
                    ...
                    <td className={ rowBg(key) + " relative hoverable"} data-id={ key } key={ uniqid() } >
                     <TCStatusLabel forTestStep={false} label={ item.tc_status_lbl } status={ item.tc_status }/>
                    </td>
                    
                </tr>
                <tr className={ (props.activeKey != "" && props.activeKey == key)  ? '' : 'hide' }>
                    <td colSpan="7"  className={ rowBg(key)+ ' hoverable' } data-id={ key }>
                        <ReviewTableRowValue data={ item.steps }  key={ uniqid() }/></td>
                </tr>
            </>
        )
    }
